...
public:
    ...
    void executeEstimateIntent(const riddle::EstimateIntentGoalConstPtr &goal)
    {
               ...
               this->tpsoWrapper->ok())
        {
            ...
            if (this->tpsoWrapper->getVad() == 1)
                bias = 0.3;
            else
                bias = 0.0;

            if (this->poseList.headPoseLists.size() > 0 )
            {
                double dy, dp, likelihood;
                dy = (this->poseList.headPoseLists[0].pose[3])*(this->poseList.headPoseLists[0].pose[3]);
                dp = (this->poseList.headPoseLists[0].pose[4])*(this->poseList.headPoseLists[0].pose[4]);

                double norm_fact = 1.0;//(2.0*M_PI*10.*20.);
                dy  = -0.5*(dy)/100.;
                dp  = -0.5*(dp)/400.;

                likelihood = exp(dy)*exp(dp)/(norm_fact);
                this->intentBel[0] = likelihood + bias;
                this->intentBel[1] = 1. - likelihood - bias;
            }
            else
            {
                this->intentBel[0] = 0.05 + bias;
                this->intentBel[1] = 0.95 - bias;
            }
