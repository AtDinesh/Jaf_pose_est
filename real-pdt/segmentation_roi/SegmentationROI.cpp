                                      ...
                                      const Vector<double>& mat_3D_points_x, const Vector<double>& mat_3D_points_y, const Vector<double>& mat_3D_points_z)
{
    ...
    for(int i = 0; i < rois.getSize(); ++i)
    {
        ...
        roi_labels(i) = Vector<int>(roi_reshaped_eqt(i).x_size()*roi_reshaped_eqt(i).y_size(), roi_reshaped_eqt(i).data()); //.data() returns a pointer to the data
        ...
        objects_data(i).setSize(roi_labels(i).getSize());
            ...
            ///////////////////////////////////////TEST///////////////////////////
//    imroi_hist.setSize(m_roisInHist.x_size(), m_roisInHist.y_size(),m_roiInHist.data());
    ...
    Vector<double>  aux(3);
    SegmentedObj* obj_ptr; //creating a pointer that will be used to point on the objects to define parameters
    int ind;
    for(int i = 0; i < indices.getSize(); ++i)
    {
        for(int j = 0; j < indices(i).getSize(); ++j)
        {
            ...
            ind = objects_data(i)(ii).min_x_ind; //define coordinates for min x,y,z points (front bottom left point) for each object
            ...
            ind = objects_data(i)(ii).max_x_ind; //define coordinates for max x,y,z points (back upper right point) for each object
            aux(0)  = mat_3D_points_x(ind);
            aux(1)  = mat_3D_points_y(ind);
            aux(2)  = mat_3D_points_z(ind);
...
void SegmentationROI::RunSegmentation(Vector<SegmentedObj> &allObj,Matrix<int> &roiInHist_view,Vector<Matrix<int> > &roi_reshaped,const Matrix<double>& histogram, Matrix<int>& labeled_hist, Vector<ROI_SEG> &rois,
                                      const Matrix<int>& mat_2D_pos_x, const Matrix<int>& mat_2D_pos_y,
                                      const Vector<double>& mat_3D_points_x, const Vector<double>& mat_3D_points_y, const Vector<double>& mat_3D_points_z)
{
    double sigmaX = Globals::sigmaX;
...
//    for (int taille_x=0; taille_x<tmp_roi.x_size(); taille_x++)
//    {
//        for (int taille_y=0; taille_y<tmp_roi.y_size(); taille_y++)
//        {
//            roiInHist_view(taille_x, taille_y) = tmp_roi(taille_x, taille_y);
//        }
//    }
...
//    std::cout << "roi_reshaped_eqt size : " << roi_reshaped_eqt.getSize() << std::endl;
//    for (int i=0;i<roi_reshaped_eqt.getSize(); i++)
//    {
//        std::cout << "object number " << i+1 << ", size : (" << roi_reshaped_eqt(i).x_size() << ","<< roi_reshaped_eqt(i).y_size() << ")" <<std::endl;
//    }

    roi_reshaped =roi_reshaped_eqt;
}
