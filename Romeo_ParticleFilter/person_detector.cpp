...
//Particle Filter Tracker headers														//PT
//#include "ParticleFilter/hogpeopledetector.h"
#include "icu_particle_filter/include/ICU_Particle_Filter/particlefilterdemonstrator.hpp"
#include <ctime>
#include <fstream>
...
/* define this variable to activate particle filter tracking									//PT
*/
//#define PARTICLE_FILTER_TRACKING
#define MOT // Multiple Object Tracking
...
bool is_first_tracking = true;                                  //For particle_filter
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        /////////////////////////Detection////////////////////////////////////////////
        Vector<Vector< double > > detected_bounding_boxes1;							// Rendre ces détections compatibles avec le filtre particulaire
        ...
        Vector<Vector< double > > detected_bounding_boxes;						// Rendre ces détections compatibles avec le filtre particulaire
        ...
        std::cout << "((line 989)size detected_bounding_box : " <<detected_bounding_boxes.getSize()<< std::endl;
        if (detected_bounding_boxes.getSize() != 0)
        {
            std::cout << "0 :" << detected_bounding_boxes(0)(0) << "; 1 :" << detected_bounding_boxes(0)(1) << "; 2 :" << detected_bounding_boxes(0)(2) << "; 3 :"<< detected_bounding_boxes(0)(3) << "; 4 :" << detected_bounding_boxes(0)(4) << std::endl;
            std::cout << "calculated end row : " << (int) detected_bounding_boxes(0)(1) + (int)(detected_bounding_boxes(0)(3)/2.0) + ((detected_bounding_boxes(0)(3)*Globals::evaluation_inc_height_ratio)/2.0) << std::endl;
        }

        //////////////////////////////////////////////////////////////////////////////

        /*///////////////////////////////////////////TRACKING///////////////////////////		Commencer le tracking par filtre particulaire ici														//Modified
        ct=CPUTime();
        ...
        if (is_first_tracking) //if it is the first tracking, we need to be sure that someone is detected then we turn this boolean to false !
        {
            if(detected_bounding_boxes.getSize() == 0)
                is_first_tracking = true;   // nobody detected --> still the first tracking !
            else
            {
                ///Convert CImg to OpenCV (needed for tracking)
                unsigned char * src_ptr_track = (unsigned char*)cim_final.data();
                ...
                for(int i = 0; i < h; i++)
                {
                    for(int j = 0; j < w; j++)
                    {
                        img_vis_track.data[(i*w + j)*3 + 0] = src_ptr_track[ i*w + j + 0*channel_sz]; //r channel
                        img_vis_track.data[(i*w + j)*3 + 1] = src_ptr_track[ i*w + j + 1*channel_sz]; //g channel
                        img_vis_track.data[(i*w + j)*3 + 2] = src_ptr_track[ i*w + j + 2*channel_sz]; //b channel
                ...
                delete[] src_ptr_track;                 //free memory

                //Create rectangles from the detected_bounding_boxes
                for ( int detection_number = 0; detection_number < detected_bounding_boxes.getSize(); ++detection_number)
                {
                    int nb_row = (int)(detected_bounding_boxes(detection_number)(3)/2.0) + ((detected_bounding_boxes(detection_number)(3)*Globals::evaluation_inc_height_ratio)/2.0);
                    cv::Rect rectangle_box(detected_bounding_boxes(detection_number)(0),detected_bounding_boxes(detection_number)(1),detected_bounding_boxes(detection_number)(2),nb_row); //Rect(int x, int y, int width, int height)
                    // Create an Observation element and push it in the detections vector

                    //Observation Obs1(img_vis_track, rectangle_box); //essayer detections.push_back(Observation(img_vis_track, rectangle_box));
                    //detections.push_back(Obs1);
                }
                //ParticleFilterDemonstrator demo; //prend en charge SOT et MOT
                //demo.step(frame, detections); //calcul des données nécessaires pour la prochaine image (instanciation tracker, association tracker/detection, mort tracker + redetection)
                //std::vector<ParticleFilter> boundingBox = demo.getTracks(); //récupération des trackers

                is_first_tracking=false;                // not the first tracking any longer !
            }
        }
//      else
//      {
//            ++global_frame_counter;
//            detections.clear(); //clear all previous detection
//
//            ///Convert CImg to OpenCV (needed for tracking)
//            unsigned char * src_ptr_track = (unsigned char*)cim_final.data();
//
...
//        cv::imshow("output", frame);
		...
		// Ajouter le tracking mode ou inclure le tracking dans les autres modes !
...
//    }
    ...
    cv::Mat img_vis_track;                                                   // to work on Particle Filter (frame)
...
Vector<Vector<double> > BboxNMS(Vector<Vector<double> >& detected_bounding_boxes, double thresh) // used only if hog defined !
{
...
int main_preload()
{
    ...
    for(int current_frame = 0; current_frame < Globals::numberFrames; current_frame++)
    {
        ...
        if(Globals::use_segmentation_roi)
        {
            ...
            detector_seg->GetROIs(cameras(current_frame), depth_maps(current_frame), labeledMap, point_cloud);
        }
        else
...
int main(int argc, char** argv)
{
    ...
    std::string data_path;
    {
        ...
        nh_t.param("data_path", data_path,
                   std::string("/home/datchuth/catkin_ws/src/riddle/data/pdt/"));
