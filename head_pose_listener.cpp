...
public:
    ...
    void headPoseReader(const riddle::HeadPoseList & msg);
    void drawHeadPose();
...
void riddle::HeadPoseVisualiser::depthMapReader(const sensor_msgs::PointCloud2ConstPtr& msg)
{
    PointCloud cloud;
...
void riddle::HeadPoseVisualiser::headPoseReader(const riddle::HeadPoseList & msg)
{
    //read poses and draw ontop of this
    this->headPoseList = msg;
...
void riddle::HeadPoseVisualiser::drawHeadPose()
{
    ...
    //draw head poses
    //if(g_means.size()>0){
    if(this->headPoseList.headPoseLists.size() > 0)
    {
        glColor3f( 0, 1, 0);
        //degree to rad conversion factor
        float mult = 0.0174532925f;

        for(unsigned int i=0;i<this->headPoseList.headPoseLists.size();++i){

            const riddle::HeadPose & p = this->headPoseList.headPoseLists[i];
...
riddle::HeadPoseVisualiser::HeadPoseVisualiser()
    :_depth_image_topic("depth_msg"), _head_pose_topic("hp_msg")
{
    _sub_depth = _nh.subscribe(_depth_image_topic, 1,
                               &HeadPoseVisualiser::depthMapReader, this);

    _sub_hpose = _nh.subscribe(_head_pose_topic, 1,
                               &HeadPoseVisualiser::headPoseReader, this);
}
...
int main(int argc, char* argv[])
{
    ...
    riddle::HeadPoseVisualiser head_pose_vis;
